...
if($w == "u"){
    ...
    if($sign_off_mng_id1 != ""){
        ...
        if($sign_off_id_info['mb_token'] != ""){ //토큰이 있는경우 푸시 발송
            ...
            if($_SERVER['REMOTE'] != ADMIN_IP) try { fcm_send($sign_off_id_info['mb_token'], $push_title, $push_content, "sign_off", "{$sign_id}", "/holiday_reqeust_info.php?mng=Y&sign_id="); } catch(Exception $e) {}
