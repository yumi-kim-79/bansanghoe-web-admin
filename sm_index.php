...
function moveCal(year, month, type, calendar_code, checkDate = ''){
    ...
    if(type == '1'){
        $.ajax({
            ...
            data: {toYear:year, toMonth:month, type:type, mb_id:mb_id, calendar_code:calendar_code, checkDate:checkDate}, 
            cache: false,
            async: true,
            contentType : "application/x-www-form-urlencoded; charset=UTF-8",
            success: function(data) {
...
$(function(){
    ...
    const month    = sessionStorage.getItem('calendar_month');
    const type     = sessionStorage.getItem('calendar_type');
    ...
    if(year && month && type){
        moveCal(year, month, type, category, calendar_date);
    }
});
</script>
<?php
