...
//일반 회원 정보 휴대폰번호로 가져오기
function get_user_hp($hp){
    $sql = "SELECT * FROM a_member
            WHERE mb_hp = '{$hp}' and is_del = 0";
    $row = sql_fetch($sql);
...
//매니저 정보
function get_manger($id){
    $sql = "SELECT mng.*, depart.md_name, grade.mg_name FROM a_mng as mng
...
//점검 카테고리
function get_industry_info($id){
    $sql = "SELECT * FROM a_industry_list WHERE industry_idx = '{$id}'";
    $row = sql_fetch($sql);
...
//관리단 정보
function get_mng_team($mb_id){
    $sql = "SELECT mt.*, mtg.gr_name, dong.dong_name, ho.ho_name FROM a_mng_team as mt
...
//빌딩 단지 정보
function get_builiding_info($id){
    $sql = "SELECT * FROM a_building WHERE building_id = '{$id}'";
    $row = sql_fetch($sql);
...
//관리단 정보
function mng_team_building_info($mt_id){
    $sql = "SELECT mt.*, mt.build_id as building_id, mtg.gr_name, building.building_name, building.building_policy FROM a_mng_team as mt
            LEFT JOIN a_mng_team_grade as mtg on mtg.gr_id = mt.mt_grade
            LEFT JOIN a_building as building on mt.build_id = building.building_id
            WHERE mt.mt_id = '{$mt_id}'";
    // echo $sql;
    $row = sql_fetch($sql);
...
//단지정보
function get_builiding($id){
    $sql = "SELECT ho.*, building.building_name, building.building_bill_account, building.building_bill_account_bank, building.building_bill_account_name, building.building_bill_notice, building.building_policy, dong.dong_name FROM a_building_ho as ho
            ...
            WHERE ho.ho_id = '{$id}' ORDER BY ho.ho_id desc";
    //echo $sql;
    $row = sql_fetch($sql);
...
//동이름
function get_dong($idx){
    $sql = "SELECT * FROM a_building_dong WHERE dong_id = '{$idx}'";
    $row = sql_fetch($sql);
...
//호수이름
function get_ho($idx){
    $sql = "SELECT * FROM a_building_ho WHERE ho_id = '{$idx}'";
    $row = sql_fetch($sql);
...
//담당 단지
function get_mng_building($id){
    $sql = "SELECT mgb.*, building.is_use FROM a_mng_building as mgb
            LEFT JOIN a_building as building on mgb.building_id = building.building_id
            WHERE mgb.is_del = 0 and mgb.mb_id = '{$id}' and building.is_use = 1 ORDER BY mgb.mng_id desc";
    // echo $sql.'<br>';
    $res = sql_query($sql);
    ...
    while($row = sql_fetch_array($res)){
        array_push($mng_building, $row['building_id']);
    }
...
//사내용 게시판 카테고리 정보
function get_bbs_category($code){
    $sql = "SELECT * FROM a_bbs_setting WHERE bbs_code = '{$code}'";
    $row = sql_fetch($sql);
...
//캘린더 카테고리 정보
function get_calendar_category($code){
    $sql = "SELECT * FROM a_calendar_setting WHERE cal_code = '{$code}'";
    $row = sql_fetch($sql);

    return $row;
}

function get_addr($post_idx){
    $sql = "SELECT * FROM a_post_addr WHERE post_idx = '{$post_idx}'";
    $row = sql_fetch($sql);
...
function building_lookup_candidates($scode, $bcode, $dong_name = ''){
    ...
    // 2026-07 신설 구 → 개편 前 소속 구(조회 후보)
    $reorg = array(
        '28275' => array('28260'),            // 서해구  ← 서구
        '28290' => array('28260'),            // 검단구  ← 서구
        '28125' => array('28110', '28140'),   // 제물포구 ← 중구 + 동구
        '28155' => array('28110'),            // 영종구  ← 중구
    );

    if (!isset($reorg[$scode])) {
        return $list; // 개편 신설 구가 아니면 폴백 불필요
    }

    foreach ($reorg[$scode] as $legacy) {
 ...
 *
 * ★행정안전부 법정동코드 전체자료(폐지 포함)에서 생성. 인천 신설 4개 구의 법정동 80개를
 *   **법정동명으로 폐지 구의 코드에 전량 매칭**했다(미매칭 0건, 불로동 12100 실조회 검증).
 *   재생성이 필요하면 같은 자료를 받아 동명 기준으로 다시 맞추면 된다.
 */
function legacy_bjdong_code($legacy_sigungu, $dong_name){
    static $map = array(
        // 인천 서구 → 서해구·검단구로 분리  (21개)
        '28260' => array(
            '가정동' => '10800', '가좌동' => '11200', '검암동' => '10300', '경서동' => '10400',
            '공촌동' => '10500', '금곡동' => '11800', '당하동' => '11400', '대곡동' => '11700',
            '마전동' => '11300', '백석동' => '10100', '불로동' => '12100', '석남동' => '11000',
            '시천동' => '10200', '신현동' => '10900', '심곡동' => '10700', '연희동' => '10600',
            '오류동' => '11900', '왕길동' => '12000', '원당동' => '11500', '원창동' => '11100',
            '청라동' => '12200',
        ),
        // 인천 중구 → 제물포구·영종구로 분리  (52개)
        '28110' => array(
            '경동' => '13400', '관동1가' => '10900', '관동2가' => '11000', '관동3가' => '11100',
            '남북동' => '15000', '내동' => '13300', '답동' => '12500', '덕교동' => '15100',
            '도원동' => '13200', '무의동' => '15200', '북성동1가' => '13800', '북성동2가' => '13900',
            '북성동3가' => '14000', '사동' => '12200', '선린동' => '14100', '선화동' => '12900',
            '송월동1가' => '14200', '송월동2가' => '14300', '송월동3가' => '14400', '송학동1가' => '11900',
            '송학동2가' => '12000', '송학동3가' => '12100', '신생동' => '12300', '신포동' => '12400',
            '신흥동1가' => '12600', '신흥동2가' => '12700', '신흥동3가' => '12800', '용동' => '13500',
            '운남동' => '14600', '운북동' => '14800', '운서동' => '14700', '유동' => '13000',
            '율목동' => '13100', '을왕동' => '14900', '인현동' => '13600', '전동' => '13700',
            '중산동' => '14500', '중앙동1가' => '10100', '중앙동2가' => '10200', '중앙동3가' => '10300',
            '중앙동4가' => '10400', '항동1가' => '11200', '항동2가' => '11300', '항동3가' => '11400',
            '항동4가' => '11500', '항동5가' => '11600', '항동6가' => '11700', '항동7가' => '11800',
            '해안동1가' => '10500', '해안동2가' => '10600', '해안동3가' => '10700', '해안동4가' => '10800',
        ),
        // 인천 동구 → 제물포구로 편입  (7개)
        '28140' => array(
            '금곡동' => '10600', '만석동' => '10100', '송림동' => '10700', '송현동' => '10300',
            '창영동' => '10500', '화수동' => '10200', '화평동' => '10400',
        ),
    );
...
function building_api2($scode, $bcode, $bun = '', $ji = ''){
    ...
    $apiUrl = "https://apis.data.go.kr/1613000/BldRgstHubService/getBrRecapTitleInfo?serviceKey=".$apiKey."&sigunguCd=".$scode."&bjdongCd=".$bcode."&bun=".$bun."&ji=".$ji."&_type=json&numOfRows=10&pageNo=1";
...
//부서이름
function get_department_name($idx){

    $sql = "SELECT * FROM a_mng_department WHERE md_idx = {$idx}";
    $row = sql_fetch($sql);
...
// 매니저 직급이름
function get_mng_grade_name($idx){

    $sql = "SELECT * FROM a_mng_grade WHERE mg_idx = {$idx}";
    $row = sql_fetch($sql);
...
//결재서류명
function approval_category_name($code){
    $sql = "SELECT * FROM a_sign_off_category WHERE sign_cate_code = '{$code}'";
    $row = sql_fetch($sql);

    return $row['sign_cate_name'];
}

//fcm v1
//1. 인증 
//firebase oauth(push)
function oauth_firebase(){
    ...
    $oauth_key = $client->fetchAccessTokenWithAssertion();

    //    $_cfg['firebase']['project_id'] = "misohae-db612";
    //     $_cfg['firebase']['project_name'] = "MISOHAE";
    //     $_cfg['firebase']['url'] = "https://fcm.googleapis.com/v1/projects/".$_cfg['firebase']['project_id']."/messages:send";
...
function aligo_sms($phoneNumber, $messageText, $type="SMS"){
    ...
    $sms['user_id'] = "smtm2220"; // SMS 아이디
    $sms['key'] = "x2zvf135q5mjmm94eb23j8kebtgvdnto";//인증키
    /****************** 인증정보 끝 ********************/

    /****************** 전송정보 설정시작 ****************/
    $_POST['msg'] = $messageText; // 메세지 내용 : euc-kr로 치환이 가능한 문자열만 사용하실 수 있습니다. (이모지 사용불가능)
    $_POST['receiver'] = $phoneNumber; // 수신번호
    $_POST['destination'] = $phoneNumber; // 수신인 %고객명% 치환
    $_POST['sender'] ="032-513-2220"; // 발신번호
...
function validatePassword($password) {
    // Regular expression: at least one letter and one number
    
    //$pattern = '/^(?=.*[A-Za-z])(?=.*\d)[A-Za-z\d\W_]+$/'; // 영문+숫자+특수문자
    $pattern = '/^(?=.*[A-Za-z])(?=.*\d)[A-Za-z0-9]+$/'; //영문+숫자
...
//페이징 ajax
function get_paging_ajax($write_pages, $cur_page, $total_page, $add = "")
{
    $str = '';

    if ($cur_page > 1) {
        $str .= '<button type="button" class="pg_page pg_page_noti pg_start" data-page="1">처음</button>'.PHP_EOL;
    }
    ...
    if ($start_page > 1) {
        $str .= '<button type="button" class="pg_page pg_page_noti pg_prev" data-page="'.($start_page-1).'">이전</button>'.PHP_EOL;
    }
    ...
    if ($total_page > 1) {
        for ($k=$start_page;$k<=$end_page;$k++) {
            ...
            $str .= '<button type="button" class="pg_page '.$page_on.' pg_page_noti" data-page="'.$k.'">'.$k.'<span class="sound_only">페이지</span></button>'.PHP_EOL;
           
        }
    }

    if ($total_page > $end_page) {
        $str .= '<button type="button" class="pg_page pg_page_noti pg_next" data-page="'.($end_page+1).'">다음</button>'.PHP_EOL;
    }

    // if ($cur_page < $total_page) {
    //     $str .= '<button type="button" class="pg_page pg_page_noti pg_end" data-page="'.$total_page.'">맨끝</button>'.PHP_EOL;
    // }
...
function get_sm_member($mb_id, $fields = '*', $is_cache = false)
{
    global $g5;
    ...
    $sql = " SELECT {$fields} from a_mng where mng_id = '{$mb_id}' ";

    echo $sql;

    $cache[$mb_id][$key] = run_replace('get_sm_member', sql_fetch($sql), $mb_id, $fields, $is_cache);
...
// 일반 로그인 패스워드 체크
function login_password_check2($mb, $pass, $hash)
{
    global $g5;
...
function resizeImage($file, $max_width, $max_height) {
    ...
    $img_sample = getimagesize($file);

    $orig_width = $img_sample[0];
    $orig_height = $img_sample[1];

    $width = $img_sample[0];
    $height = $img_sample[1];
    $mime = $img_sample['mime'];
    ...
    $mime_array = ['image/jpeg', 'image/gif', 'image/png'];
    ...
    if(in_array($mime, $mime_array)){
        ...
        // 압축된 이미지 저장
        imagejpeg($image_p, $file, 75); // 75는 품질을 나타내며, 0에서 100 사이의 값입니다.
    }
    //return $img_files;
}
