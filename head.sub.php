...
$viewport = '<meta name="viewport" id="meta_viewport" content="width=device-width,initial-scale=1.0,minimum-scale=0,maximum-scale=1.0">'; // 기본값
...
add_javascript('<script src="https://code.jquery.com/jquery-3.5.1.min.js"></script>', 0);
...
//일반 회원인지
//회원이라면 회원정보 가져오고 회원임을 표시
if($_SESSION['users']['id'] != ""){
    $user_info = get_user($_SESSION['users']['id']);

    if($user_info['mb_type'] == "OUT"){
        $user_building = mng_team_building_info($_SESSION['users']['mng_building']);

    }else{
        $user_building = get_builiding($_SESSION['users']['ho_id']);
    }
}
...
$is_users = $_SESSION['users']['id'] != "" ? 1 : 0;


//일반 회원 로그인상태인데 회원 탈퇴라면
if($is_users){
    if($user_info['is_del'] == 1){

        $sql= " update a_member set mb_auto = '0', mb_token = '' where mb_id = '{$user_info['mb_id']}' ";
        sql_query($sql);

        unset($_SESSION["users"]); //로그아웃 처리

        echo "<script>document.location='/bbs/login.php';</script>";
    }
}

//매니저 회원 로그인상태인데 회원 탈퇴라면
if($is_member){
    if($mng_info['is_del'] == 1){

        $sql= " update g5_member set mb_auto = '0', mb_token = '' where mb_id = '{$mng_info["mb_id"]}' ";
        sql_query($sql);
...
//if($is_users) print_r2($user_info);
// if($is_users) print_r2($user_building);