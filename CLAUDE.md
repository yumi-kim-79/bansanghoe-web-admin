...
- [x] **온라인 투표 템플릿 선택 UI를 인라인 검색 드롭다운으로 변경** (2026-04-02)
  ...
  - CHEditor5 인스턴스명: `ed_vt_content` (editor_html('vt_content',...) → `ed_{$id}`)
  - `replaceContents()` → fallback `doc.body.innerHTML` 직접 삽입, 상세 디버그 로그 포함
  - tplClearSelection()도 CHEditor5 `replaceContents('')`로 초기화, `#tx_vt_content` 동기화
