...
### 최근 완료
- [x] **SMS 포커스 감지 자동 진행 방식으로 전환** (2026-04-15)
  - setTimeout 타이머 완전 제거 → `visibilitychange` 이벤트로 앱 복귀 감지
  - 문자앱에서 발송 후 돌아오면 0.5초 후 자동으로 다음 대상 발송
  - 발송 중단 버튼 추가, 현재 발송 대상 초록색 카드 표시
  - 목록 자동 스크롤, 하이라이트, 중단 시 진행률 표시
