...
### 최근 완료
- [x] **SMS 자동 순차 개별 발송 방식으로 전환** (2026-04-15)
  - `sms_send_sm.php`: 그룹 분할/번호 복사 방식 완전 제거 → 자동 순차 개별 발송으로 전환
  - "자동 순차 발송 시작" 버튼 1개, 3초 간격 sms: URI 순차 호출
  - 프로그레스바 + 현재 발송 대상 실시간 표시 + 목록 하이라이트
  - 개별 SMS로 발송되므로 수신자끼리 서로 보이지 않음, 인원수 제한 없음
