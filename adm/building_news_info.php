...
$sql_search = " where (1) and bb.is_del = '0' and bb.bbs_type = 'infomation' ";
...
if($is_submit){
    $sql_search .= " and bb.is_submit = '{$is_submit}' ";

    $qstr .= '&is_submit='.$is_submit;
}
    ...
    <div class="serach_box">
        ...
        <div class="sch_selects ver_flex gap15">
            <div class="sch_radios">
                <input type="radio" name="is_submit" id="status1" value="" <?php echo $is_submit == "" ? "checked" : "" ?>>
                <label for="status1">전체</label>
            </div>
            <div class="sch_radios">
                <input type="radio" name="is_submit" id="status2" value="N" <?php echo $is_submit == "N" ? "checked" : "" ?>>
                <label for="status2">발행전</label>
            </div>
            <div class="sch_radios">
                <input type="radio" name="is_submit" id="status3" value="S" <?php echo $is_submit == "S" ? "checked" : "" ?>>
                <label for="status3">발행</label>
            </div>
            <div class="sch_radios">
                <input type="radio" name="is_submit" id="status4" value="R" <?php echo $is_submit == "R" ? "checked" : "" ?>>
                <label for="status4">회수</label>
            </div>
        </div>
    </div>
    <div class="serach_box">
    ...
    <div class="serach_box">
        ...
        <div class="sch_selects ver_flex">
            <select name="sfl" id="sfl" class="bansang_sel">
                ...
                <option value="bb_number" <?php echo get_selected($sfl, "bb_number"); ?>>문서번호</option>
...
<form name="fbuilindbbslist" id="fbuilindbbslist" action="./building_news_info_list_update.php" onsubmit="return fbuilindbbslist_submit(this);" method="post">
    ...
    <input type="hidden" name="bbs_type" value="infomation">
        ...
        <table>
            ...
            <tbody>
                ...
                for ($i = 0; $row = sql_fetch_array($result); $i++) {
                    ...
                    // if($_SERVER['REMOTE_ADDR'] == ADMIN_IP) print_r2($row);
                ?>
                    ...
                    <tr class="<?php echo $bg; ?>">
                        ...
                        <td><?php echo $row['building_id'] != '-1' ? $row['building_name'] : '전체'; ?></td>
                        <td><?php echo $row['bb_number']; ?></td>
                        <td><?php echo $row['bb_title']; ?></td>
                        <td>
                            <?php 
                            // echo $row['mb_name'];
                                $mngs = get_manger($row['wid']);
                                echo $mngs['md_name'];
                            ?>
                        </td>
                        ...
                        <td>
                            <?php
                            echo $row['bbs_gigan'] ? '영구게시' : $row['sdate'].' ~ '.$row['edate'];
                             ?>
                        </td>
                        <td>
                            <?php
                        ...
                        <td headers="mb_list_mng" class="td_mng td_mng_s">
                            <a href="./building_news_info_form.php?<?=$qstr;?>&amp;w=u&amp;bb_id=<? echo $row['bb_id']; ?>" class="btn btn_03">관리</a>
                        </td>
                    </tr>
                <?php
                }
    ...
    <div class="btn_fixed_top">
        ...
        <?php if ($is_admin == 'super') { ?>
            <a href="./building_news_info_form.php" id="member_add" class="btn btn_03">안내문 등록</a>
        <?php } ?>
...
function fbuilindbbslist_submit(f) {
    ...
    if (document.pressed == "선택삭제") {
        if (!confirm("선택한 안내문을 정말 삭제하시겠습니까?")) {
            return false;
        }
    }
