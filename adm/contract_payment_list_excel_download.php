...
WHERE ct.ct_idx IN ($ct_idx_arr) and ct.is_del = 0 and ct.is_temp = 0 and ct.ct_status = 0 and (ch.ct_sdate <= '{$month_end2}' and ch.ct_edate >= '{$month_start2}') 
GROUP BY ct.ct_idx 
ORDER BY ct.company_name asc, building.building_name asc, ct.ct_idx desc";
...
foreach($res as $row){
    ...
    $pl_dates = $pl_row['is_services'] ? '서비스' : $pl_row['payment_date'];

   
    //echo $c_bills;
    //echo $c_bills_rows['bill_dates'];
    $bill_memos = $pl_row['payment_status'] == '4' ? nl2br($pl_row['payment_memo']) : '';

    //$bill_memos = $c_bills_rows['bill_statusm'] == '3' ? nl2br($pl_row['bills_memo']) : '';
    ...
    $sheet->setCellValue('E'.$cell, $pl_dates);
...
$filename = "신반상회_지급 리스트_" . date('Ymd') . ".xlsx";
