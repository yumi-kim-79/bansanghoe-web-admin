    ...
    if ($file['error'] === UPLOAD_ERR_OK) {
        ...
        try {
            ...
            $html .= '<style>
                    ...
                    .group-header {
                        background-color: #f0f0f0; /* 그룹 헤더 배경색 */
                        font-weight: bold;
                        text-align: left;
                        padding: 10px;
                    }
            ...
            //exit;
             ...
             foreach ($groupedData as $groupKey => $rows) {
                ...
                $ho_sql_add = "SELECT ho.ho_name FROM 
                                ...
                                WHERE dong.building_id = '{$building_id}' and dong.dong_name = '{$dongs}' ORDER BY ho.dong_id asc, ho.ho_name + 1 asc";
                // echo $ho_sql_add.'<br>';
                ...
                // print_r2($ho_diff);
                 ...
                 if(count($ho_diff) > 0){
                    ...
                    usort($merged, function($a, $b) {
                        ...
                        // "동합계"와 "전체합계"는 항상 맨 뒤로
                        if ($a === '동합계' || $a === '전체합계') return 1;
                        if ($b === '동합계' || $b === '전체합계') return -1;

                        // "동 호"는 항상 맨 앞으로
                        if ($a === '동 호') return -1;
                        if ($b === '동 호') return 1;

                        // 숫자와 하이픈 포함된 값들을 비교 (예: 502, 502-1)
