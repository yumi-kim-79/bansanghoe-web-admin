...
function get_member_id_select($name, $level, $selected = "", $event = "")
{
    ...
    $str = '<select id="' . $name . '" name="' . $name . '" ' . $event . ' class="bansang_sel"><option value="">선택안함</option>';
...
// 접근 권한 검사
if (!$member['mb_id']) {
    alert('로그인 하십시오.', G5_BBS_URL . '/admin_login.php?url=' . urlencode(correct_goto_url(G5_ADMIN_URL)));
