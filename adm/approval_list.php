...
$sql_common = " from a_sign_off as so
                left join g5_member as mb on so.mng_id = mb.mb_id ";
...
if($mng_infos['mng_certi'] != 'D'){
    $sql_search = " where (1) and so.is_del = '0' ";
}else{
    $sql_search = " where (1) and so.is_del = '0' and so.mng_id = '{$mb_ids}' ";
}
...
if($sdate != "" && $edate == ""){
    $sql_search .= " and so.wdate >= '{$sdate}' ";

    $qstr .= '&sdate='.$sdate;
}else if($sdate == "" && $edate != ""){
    $sql_search .= " and so.wdate <= '{$edate}' ";

    $qstr .= '&edate='.$edate;
}else if($sdate != "" && $edate != ""){
    $sql_search .= " and (so.wdate >= '{$sdate}' and so.wdate <= '{$edate}')";

    $qstr .= '&sdate='.$sdate.'&edate='.$edate;
}

if($sign_off_category){
    $sql_search .= " and so.sign_off_category = '{$sign_off_category}' ";
...
if($sign_off_status){
    $sql_search .= " and so.sign_status = '{$sign_off_status}' ";

    $qstr .= '&sign_off_status='.$sign_off_status;
}

if($mng_department){
    $sql_search .= " and so.mng_department = '{$mng_department}' ";
...
$sql_order = " order by so.sign_id desc ";
...
$sql = " select so.*, mb.mb_name {$sql_common} {$sql_search} {$sql_search2} {$sql_order} limit {$from_record}, {$rows} ";
...
<form name="fstudentlist" id="fstudentlist" action="./student_list_update.php" onsubmit="return fstudentlist_submit(this);" method="post">
                ...
                ?>
                    ...
                    <tr class="<?php echo $bg; ?>">
                        ...
                        <td headers="mb_list_mng" class="td_mng td_mng_s">
                            <a href="./approval_form.php?<?=$qstr;?>&amp;w=u&amp;sign_id=<? echo $row['sign_id']; ?>" class="btn btn_03">관리</a>
                        </td>
                    </tr>
                <?php
                }
    ...
    <div class="btn_fixed_top">
        <?php if ($is_admin == 'super') { ?>
            <a href="./approval_form.php" id="member_add" class="btn btn_03">결재 등록</a>
        <?php } ?>
    </div>
