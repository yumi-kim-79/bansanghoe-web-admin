...
<?php }?>
<form name="fcalendar" id="fcalendar" action="./calendar_form_update.php" onsubmit="return fcalendar_submit(this);" method="post" enctype="multipart/form-data">
            ...
            <tbody>
                    ...
                    <td colspan='3'>
                        <?php if($row['is_process']){?>
                            <div class="process_p mgb5">
                                <?php echo $row['processed_at']; ?> 처리완료
                            </div>
                        <?php }?>
                        <button type="button" onclick="calendar_process();" class="btn <?php echo $row['is_process'] ? 'btn_02' : 'btn_03';?>" <?php echo $row['is_process'] ? 'disabled' : '';?>>처리완료</button>
                    </td>
                </tr>
                <?php }?>
                <tr>
                    <th>캘린더종류</th>
                    <td colspan="3">
                        <select name="cal_code" id="cal_code" class="bansang_sel arr93" required onchange="calCodeChange();" <?php echo $row['is_process'] ? 'readonly' : '';?>>
                ...
                <tr class="tr_show">
                    ...
                    <td>
                        ...
                        <select name="post_id" id="post_id" class="bansang_sel" onchange="post_change();"  <?php echo $row['is_process'] ? 'readonly' : '';?>>
                    ...
                    <td colspan='3'>
                        ...
                        <div class="sch_box_wrap ">
                            <div class="sch_box_left">
                                ...
                                <!-- 검색어를 입력해주세요. -->
                                <input type="text" name="building_sch" id="building_sch" class="bansang_ipt <?php echo $row['is_process'] ? '' : 'ver2';?>" size="50" placeholder="단지명을 입력하세요." <?php echo $row['is_process'] ? 'readonly' : '';?>>
                            </div>
                           
                        </div>
                        <input type="hidden" name="building_id" id="building_id" value="<?php echo $row['building_id']; ?>">
                        <input type="text" name="building_name" id="building_name" class="bansang_ipt <?php echo $row['complain_status'] == 'CD' ? '' : 'ver2';?> mgt10" size="100" placeholder="선택한 단지가 보여집니다." readonly value="<?php echo $build_row['building_name']; ?>"  >
                        <!-- required 필수제거 -->
                        <script>
                ...
                <tr class="tr_show">
                    ...
                    <td>
                        ...
                        ?>
                        <!-- required 필수제거 250908 -->
                        <select name="mng_department" id="mng_department" class="bansang_sel" onchange="department_change();"  <?php echo $row['is_process'] ? 'readonly' : '';?>>
                            <option value="">선택</option>
                    ...
                    <td>
                        ...
                        <select name="mng_id" id="mng_id" class="bansang_sel" <?php echo $row['is_process'] ? 'readonly' : '';?>>
                            <option value="">선택</option>
                            <?php if($row['mng_id'] == '-1'){?>
                    ...
                    <td>
                        <div class="ipt_date_boxs_wrap">
                            <div class="ipt_date_boxs">
                                <!-- readonly -->
                                <input type="text" name="cal_date" class="bansang_ipt <?php echo $row['is_process'] ? 'ipt_date_not' : 'ver2 ipt_date';?> ipt_date_cal" value="<?php echo $row['cal_date']; ?>"  required <?php echo $row['is_process'] ? 'readonly' : '';?>>
               ...
               <tr class="tr_show">
                  <th>제목</th>
                  <td colspan="3">
                    <input type="text" name="cal_title" id="cal_title" class="bansang_ipt <?php echo $row['is_process'] ? '' : 'ver2';?>" size="100" value="<?php echo $row['cal_title']; ?>" required <?php echo $row['is_process'] ? 'readonly' : '';?>>
                  </td>
               </tr>
               <tr class="tr_show">
                    <th>내용</th>
    ...
    <div class="btn_fixed_top">
        <?php
        $toYear = date("Y", strtotime($row['cal_date']));
        $toMonth = date("m", strtotime($row['cal_date']));
        ?>
