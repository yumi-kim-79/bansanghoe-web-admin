...
//alert('잘못된 접근입니다.', './contract_list_new.php');
...
function loadTableData() {
  ...
  xhr.open("POST", "load_table_data.php");
...
<div class="cm_pop" id="contract_payment_prg_pop">
    ...
    <div class="cm_pop_cont ver_pop ver2">
        ...
        $contract_sql = "SELECT ch.ct_sdate as ct_sdate2, ch.ct_edate as ct_edate2, ct.*, building.is_use, building.building_name, mc.company_bank_name, mc.company_account_number, mc.company_account_name FROM a_contract_history as ch 
                         ...
                         WHERE ct.is_del = 0 and ct.is_temp = 0 and ct.ct_status = 0 and (ch.ct_sdate <= '{$today}' and ch.ct_edate >= '{$today}') and building.is_use = 1 GROUP BY ct.ct_idx ORDER BY ct.company_name asc, ct.ct_idx desc";
                        ...
                        foreach($contract_res as $row){
                            ...
                            if(!isset($printed_names[$row['company_idx']])){?>
                                <td rowspan="<?php echo $contract_count[$row['company_idx']]?>">
                                    ...
                                    //and ct.building_id = '{$row['building_id']}'
                                    $ct_sqls = "SELECT ct.ct_idx, ct.ct_price, pl.is_services FROM a_contract as ct
                                          ...
                                          WHERE ct.company_idx = '{$row['company_idx']}'  and ct.is_temp = 0 and ct.ct_status = 0";
                                    // echo $ct_sqls.'<br>';
