...
if ($stx) {
    ...
    switch ($sfl) {
        ...
        case 'ho_tenant_hp':
            ...
            break;
        case 'car_type_list':
            $sql_search2 .= " HAVING car_type_list like '%{$stx}%' ";
            break;
        case 'car_name_list':
            $sql_search2 .= " HAVING car_name_list like '%{$stx}%' ";
            break;
    }
    if($sfl != "car_type_list" && $sfl != "car_name_list"){
        $sql_search .= " ) ";
...
$sql_order = " order by building.building_name asc, building.building_id asc, CAST(ho.ho_name AS UNSIGNED) ASC, ho.ho_id desc ";



// $sql = " select count(*) as cnt {$sql_common} {$sql_search} {$sql_search2} {$sql_order} ";
//HAVING car_name_list IS NOT NULL AND car_name_list != ''
$sql = " select ho.ho_id, ho.ho_name, ho.ho_tenant, ho.ho_tenant_hp, post.post_name, building.building_name, building.is_use, dong.dong_name, GROUP_CONCAT(car.car_name ORDER BY car.car_id asc SEPARATOR ', ') as car_name_list, GROUP_CONCAT(car.car_type ORDER BY car.car_id asc SEPARATOR ', ') as car_type_list {$sql_common} {$sql_search} GROUP BY ho.ho_id  {$sql_search2} {$sql_order} ";
// echo $sql.'<br>';
...
//HAVING car_name_list IS NOT NULL AND car_name_list != ''
$sql = " select ho.ho_id, ho.ho_name, ho.ho_tenant, ho.ho_tenant_hp, post.post_name, building.building_name, building.is_use, dong.dong_name, GROUP_CONCAT(DISTINCT car.car_name ORDER BY car.car_id asc SEPARATOR ', ') as car_name_list, GROUP_CONCAT(DISTINCT car.car_type ORDER BY car.car_id asc SEPARATOR ', ') as car_type_list {$sql_common} {$sql_search} GROUP BY ho.ho_id  {$sql_search2} {$sql_order} limit {$from_record}, {$rows} ";
