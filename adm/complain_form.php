...
//상태
$status_sql = "SELECT * FROM a_complain_status WHERE is_del = 0 ORDER BY cs_idx asc";
$status_res = sql_query($status_sql);

if($w == 'u'){
    $sql = "SELECT * FROM a_online_complain 
    ...
    //동
    $dong_sql = "SELECT * FROM a_building_dong WHERE building_id = '{$row['building_id']}'";
    $dong_res = sql_query($dong_sql);

    //호수
    $ho_sql = "SELECT * FROM a_building_ho WHERE building_id = '{$row['building_id']}' and dong_id = '{$row['dong_id']}'";
    $ho_res = sql_query($ho_sql);

    

    //첨부사진
    $complain_file_sql = "SELECT * FROM g5_board_file WHERE bo_table = 'complain' and wr_id = {$complain_idx} and bf_file != '' order by bf_no asc";
    $complain_file_res = sql_query($complain_file_sql);
    $complain_file_cnt = sql_num_rows($complain_file_res);
    $complain_file_max = 5;
    ...
    $complain_add_file_max = 5;

    if($_SERVER['REMOTE_ADDR'] == ADMIN_IP){
        echo $sql.'<br>';
        echo $complain_add_file_sql.'<br>';
...
<form name="fcomplain" id="fcomplain" action="./student_form_update.php" onsubmit="return fcomplain_submit(this);" method="post" enctype="multipart/form-data">
        ...
        <table>
            ...
            <tbody>
                    ...
                    <td colspan="3">
                        <div class="radio_chk_wrap">
                            <div class="radio_chk_box">
                                <input type="radio" name="complain_type" id="type1" value="admin" <?php echo $row['complain_type'] == "admin" || $w == '' ? "checked" : ""?> <?php echo $row['complain_type'] == "user" ? "disabled" : ""?>>
                                <label for="type1">관리자 접수 민원</label>
                            </div>
                            <div class="radio_chk_box">
                                <input type="radio" name="complain_type" id="type2" value="user" <?php echo $row['complain_type'] == "user" ? "checked" : ""?> disabled>
                                <label for="type2">앱 접수 민원</label>
                            </div>
                        </div>
                    </td>
                </tr>
                <tr>
                    <th>지역</th>
                    <td colspan="3">
                        <?php if($w == 'u' && $row['complain_type'] == "user"){
                            $addr_name = get_addr($row['post_id']);
                        ?>
                            <input type="hidden" name="post_id" id="post_id" class="bansang_ipt" value="<?php echo $row['post_id']; ?>">
                            <input type="text" name="post_name" id="post_name" class="bansang_ipt" value="<?php echo $addr_name['post_name'];?>" readonly>
                        <?php }else{?>
                        <select name="post_id" id="post_id" class="bansang_sel" onchange="post_change();" required <?php echo $row['complain_status'] == 'CD' ? 'readonly' : '';?>>
                ...
                <tr>
                    ...
                    <td colspan="3">
                        <?php if($w == 'u' && $row['complain_type'] == "user"){?>
                            ...
                            <input type="text" name="building_name" id="building_name" class="bansang_ipt" size="100" placeholder="선택한 단지가 보여집니다." readonly value="<?php echo $build_row['building_name']; ?>" readonly>
                        <?php }else{ ?>
                        <div class="sch_box_wrap ">
                            <div class="sch_box_left">
                                ...
                                <!-- 검색어를 입력해주세요. -->
                                <input type="text" name="building_sch" id="building_sch" class="bansang_ipt <?php echo $row['complain_status'] == 'CD' ? '' : 'ver2';?>" size="50" placeholder="단지명을 입력하세요." <?php echo $row['complain_status'] == 'CD' ? 'readonly' : '';?>>
                            </div>
                           
                        </div>
                        <input type="hidden" name="building_id" id="building_id" value="<?php echo $row['building_id']; ?>">
                        <input type="text" name="building_name" id="building_name" class="bansang_ipt <?php echo $row['complain_status'] == 'CD' ? '' : 'ver2';?> mgt10" size="100" placeholder="선택한 단지가 보여집니다." readonly value="<?php echo $build_row['building_name']; ?>" required >
                        <?php }?>
                    </td>
                </tr>
                <tr>
                    ...
                    <td>
                        <?php if($w == 'u' && $row['complain_type'] == "user"){
                            $dong_name = get_dong($row['dong_id']);
                        ?>
                            <input type="hidden" name="dong_id" id="dong_id" value="<?php echo $row['dong_id'];?>">
                            <input type="text" name="dong_name" class="bansang_ipt" value="<?php echo $dong_name['dong_name'];?>" readonly>
                        <?php }else {?>
                        <select name="dong_id" id="dong_id" class="bansang_sel" onchange="dong_change();" <?php echo $row['complain_status'] == 'CD' ? 'readonly' : '';?>>
                    ...
                    <td>
                        <!-- onchange="ho_change();" -->
                        <?php if($w == 'u' && $row['complain_type'] == "user"){
                            $ho_name = get_ho($row['ho_id']);
                        ?>
                            <input type="hidden" name="ho_id" id="ho_id" value="<?php echo $row['ho_id'];?>">
                            <input type="text" name="ho_name" class="bansang_ipt" value="<?php echo $ho_name['ho_name'];?>" readonly>
                        <?php }else{ ?>
                        <select name="ho_id" id="ho_id" class="bansang_sel"  <?php echo $row['complain_status'] == 'CD' ? 'readonly' : '';?>>
                            <option value="">선택</option>
                            <?php if($w == 'u'){?>
                                <option value="-1" <?php echo $row['ho_id'] == '-1' ? 'selected' : ''; ?>>공용부</option>
                            <?php }?>
                ...
                <tr>
                    ...
                    <td colspan="3">
                        <?php if($w == 'u' && $row['complain_type'] == "user"){?>
                        <input type="text" name="wdate" id="wdates" class="bansang_ipt" value="<?php echo $row['wdate']; ?>" readonly>
                        <?php }else{ ?>
                        <input type="text" name="wdate" id="wdates" class="bansang_ipt <?php echo $row['complain_status'] == 'CD' ? 'ipt_date_not' : 'ver2 ipt_date';?>" value="<?php echo $w == "u" ? $row['wdate'] : date("Y-m-d") ?>" <?php echo $row['complain_status'] == 'CD' ? 'readonly' : '';?>>
                        <?php }?>
                    </td>
                </tr>
                <tr>
                    ...
                    <td>
                        <?php if($w == 'u'){?>
                        <input type="hidden" name="complain_id" id="complain_id" value="<?php echo $row['complain_id']; ?>" >
                        <?php }?>
                        <input type="text" name="complain_name" id="complain_name" class="bansang_ipt <?php echo $row['complain_status'] == 'CD' ? '' : 'ver2';?>" value="<?php echo $row['complain_name']; ?>" <?php echo $row['complain_status'] == 'CD' ? 'readonly' : '';?>>
                    </td>
                    <th>민원인 연락처</th>
                    <td>
                        <input type="tel" name="complain_hp" id="complain_hp" class="bansang_ipt <?php echo $row['complain_status'] == 'CD' ? '' : 'ver2';?> phone" value="<?php echo $row['complain_hp']; ?>" maxlength="13" <?php echo $row['complain_status'] == 'CD' ? 'readonly' : '';?>>
                    </td>
                </tr>
                <tr>
                    <th>담당자 부서</th>
                    <td>
                        ...
                        <script>
                            function dapart_change(){
                                ...
                                url : "./mng_select_ajax2.php", //ajax 통신할 파일
                                type : "POST", // 형식
                                data: { "departValue":departValue}, //파라미터 값
                    ...
                    <td>
                        ...
                        $mng_sql = "SELECT mng.*, mng_grade.mg_name FROM
                                        ...
                                        WHERE mng.mng_department = '{$row['mng_department']}' and mng.is_del = 0 and mng.mng_status = 1 ORDER BY mng.mng_certi desc, mng.mng_grades desc, mng.mng_idx asc";
                        //echo $mng_sql;
                        if($_SERVER['REMOTE_ADDR'] == ADMIN_IP){
                            echo $mng_sql;
                        }
                        ...
                        ?>
                        <?php if($row['mng_id'] != ''){?>
                        <input type="hidden" name="mng_id_or" id="mng_id_or" value="<?php echo $row['mng_id']; ?>">
                        <?php }?>
                        <select name="mng_id" id="mng_id" class="bansang_sel" <?php echo $row['complain_status'] == 'CD' ? 'readonly' : '';?> onchange="mng_id_change();">
                            <option value="">선택하세요</option>
                            <?php while($mng_row = sql_fetch_array($mng_res)){?>
                            <option value="<?php echo $mng_row['mng_id']; ?>" <?php echo get_selected($row['mng_id'], $mng_row['mng_id']); ?>><?php echo $mng_row['mng_name'].' '.$mng_row['mg_name'];?></option>
                            <?php }?>
                        </select>

                        <script>
                            function mng_id_change(){
                                let mng_id_or = "<?php echo $row['mng_id']; ?>"; //기존 선택 매니저

                                var mngSelect = document.getElementById("mng_id");
                                var mngValue = mngSelect.options[mngSelect.selectedIndex].value;

                                if(mng_id_or != "" && mng_id_or != mngValue){
                                    // $.ajax({

                                    // url : "./complain_mng_change.php", //ajax 통신할 파일
                                    // type : "POST", // 형식
                                    // data: {}, //파라미터 값
                                    // success: function(msg){ //성공시 이벤트
                                    //     console.log(msg);
                                    ...
                                    //     $(".mng_change_memo_tr").html(msg);
                                    // }

                                    // });
                                    $("#mng_change_memo").attr('required', true);
                                    $("#mng_change_memo").attr('readonly', false);
                                    $("#mng_change_memo").addClass('ver2');

                                }else{
                                    // $(".mng_change_memo_tr").html("");

                                    $("#mng_change_memo").attr('required', false);
                                    $("#mng_change_memo").attr('readonly', true);
                                    $("#mng_change_memo").removeClass('ver2');
                                }
                            }
                        </script>
                    </td>
                ...
                <tr>
                    ...
                    <td>
                        ...
                        $selected_status = "";
                        if($w == 'u'){
                            $selected_status = get_selected($row['complain_status'], $status_row['cs_code']);
                        }else{

                            if($status_row['cs_code'] == "CA"){
                                $selected_status = "selected";
                            }
                        }
                        
                         ?> 
                        <select name="complain_status" id="complain_status" class="bansang_sel" <?php echo $row['complain_status'] == 'CD' ? 'readonly' : '';?>>
                            <?php for($i=0;$status_row = sql_fetch_array($status_res);$i++){?>
                                <option 
                                value="<?php echo $status_row['cs_code']; ?>"
                                <?php echo get_selected($row['complain_status'], $status_row['cs_code']); ?>
                                >
                                <?php echo $status_row['cs_name']; ?>
                                </option>
                            <?php }?>
                        </select>
                    </td>
                ...
                <tr>
                    <th>민원제목</th>
                    <td colspan="3"><input type="text" name="complain_title" id="complain_title" class="bansang_ipt <?php echo $row['complain_status'] == 'CD' ? '' : 'ver2';?>" value="<?php echo $row['complain_title']; ?>" size="100" <?php echo $row['complain_status'] == 'CD' ? 'readonly' : '';?>></td>
                </tr>
                <tr>
                    <th>민원내용</th>
                    <td colspan="<?php echo $w == 'u' ? '' : '3'; ?>">
                        <textarea name="complain_content" id="complain_content" class="bansang_ipt <?php echo $row['complain_status'] == 'CD' ? '' : 'ver2';?> full ta" <?php echo $row['complain_status'] == 'CD' ? 'readonly' : '';?>><?php echo $row['complain_content']; ?></textarea>
                    </td>
                    <?php if($w == "u"){?>
                    <th>민원 답변</th>
                    <td>
                    <textarea name="complain_answer" id="complain_answer" class="bansang_ipt <?php echo $row['complain_status'] == 'CD' ? '' : 'ver2';?> full ta" <?php echo $row['complain_status'] == 'CD' ? 'readonly' : '';?>><?php echo $row['complain_answer'];?></textarea>
                    </td>
                    <?php }?>
                </tr>
                <tr>
                    <th>첨부사진</th>
                    <td style="<?php echo $w == 'u' ? 'width:550px;' : '';?>" colspan="<?php echo $w == 'u' ? '' : '3'; ?>">
                        <?php echo help('사진은 최대 5장까지 등록가능합니다.'); ?>
                        <div class="ipt_box ver_fix">
                            <div class="img_upload_wrap img_upload_wrap1">
                                <?php if($row['complain_status'] != 'CD'){?>
                                <div class="img_upload_box ver44 ver1">
                                    ...
                                    <?php for($i=0;$complain_file_row = sql_fetch_array($complain_file_res);$i++){?>
                                        <div class="img_upload_box_wrapper4">
                                            <div class="img_upload_box ver4 filebox">
                                                ...
                                                <img src="/data/file/complain/<?php echo $complain_file_row['bf_file']; ?>" class="img_up<?php echo $i + 1;?>" alt="" onclick="bigSize('/data/file/complain/<?php echo $complain_file_row['bf_file']; ?>')">
                                              
                                                <?php if($row['complain_type'] == 'admin'){?>
                                                <div class="file_del">
                    ...
                    <td>
                        <?php echo help('사진은 최대 5장까지 등록가능합니다.'); ?>
                        <div style="<?php echo $w == 'u' ? 'width:550px;' : '';?>" class="ipt_box ver_fix">
                            <div class="img_upload_wrap img_upload_wrap2">
                                ...
                                <?php }?>
                                <?php if($w=="u" && $complain_answer_file_res){ ?>
                                    <?php for($i=0;$complain_answer_file_row = sql_fetch_array($complain_answer_file_res);$i++){?>
                                        <div class="img_upload_box_wrapper4">
                                            <div class="img_upload_box ver4 filebox2">
                                                ...
                                                <img src="/data/file/complain_answer/<?php echo $complain_answer_file_row['bf_file']; ?>" class="img_a_up<?php echo $i + 1;?>" alt="" onclick="bigSize('/data/file/complain_answer/<?php echo $complain_answer_file_row['bf_file']; ?>')">
                                            ...
                                            <?php if($row['complain_status'] != 'CD'){?>
                                            <label class="img_labels" for="img_a_up<?php echo $i + 1;?>">
                                            이미지 첨부
                                            </label>
                                            <?php }?>
                                        </div>
        ...
        <table>
            ...
            <tr>
                ...
                <td colspan="3">
                    <?php echo help('사진은 최대 5장까지 등록가능합니다.'); ?>
                    <div class="ipt_box">
                        <div class="img_upload_wrap img_upload_wrap3">
                            ...
                            <?php if($w=="u" && $complain_add_file_res){ ?>
                                <?php for($i=0;$complain_add_file_row = sql_fetch_array($complain_add_file_res);$i++){?>
                                    <div class="img_upload_box_wrapper4 img_upload_box_wrapper8">
                                        <div class="img_upload_box ver4 filebox3">
                                            ...
                                            <img src="/data/file/complain_add/<?php echo $complain_add_file_row['bf_file']; ?>" class="img_add_up<?php echo $i + 1;?>" alt="" onclick="bigSize('/data/file/complain_add/<?php echo $complain_add_file_row['bf_file']; ?>')">
                                        ...
                                        <label class="img_labels" for="img_add_up<?php echo $i + 1;?>">
                                        이미지 첨부
                                        </label>
                                    </div>
                                <?php }?>
    ...
    <div class="btn_fixed_top">
        <a href="./complain_list.php?type=<?php echo $type?>&<?php echo $qstr ?>" class="btn btn_02">목록</a>
        <?php if($type == "progress"){?>
        <!-- <input type="submit" value="저장" class="btn_submit btn btn_02" accesskey='s'> -->
        <button type="button" class="btn_submit btn btn_03" onclick="fcomplain_submit();">저장</button>
        <?php }?>
    </div>
</form>
