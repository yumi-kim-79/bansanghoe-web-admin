        ...
        <script>
            $(document).on("keyup", ".building_name_sch", function(){
                ...
                if(sch_text != ""){
                    ...
                    $.ajax({
                        url : "./building_mng_sch_text.php",
                        type : "POST",
                        data: { "sch_category":"building_name", "sch_text":sch_text, "type":"Y", "post_id":post_id},
                        success: function(msg){ $(".sch_result_box1").html(msg); }
                    })
                }else{
            ...
            function sch_handler(text, bid){
                ...
                $("#building_id").val(bid);
                $.ajax({
                    url : "./building_mng_sch_ho_dong.ajax.php",
                    type : "POST",
                    data: { "building_id":bid},
                    success: function(msg){ $("#dong_id").html(msg); }
                })
            }
        </script>
    </div>
    <div class="serach_box">
        ...
        <div class="sch_selects ver_flex building_dong_ho">
            ...
            <select name="ho_id" id="ho_id" class="bansang_sel">
                ...
                <?php while($row_ho = sql_fetch_array($res_ho)){ ?>
        ...
        <script>
            function dong_change(){
                ...
                $.ajax({
                    url : "./building_ho_ajax.php",
                    type : "POST",
                    data: { "dong_id":dongValue},
                    success: function(msg){ $("#ho_id").html(msg); }
                });
            }
        </script>
    </div>
            ...
            <tbody>
                ...
                ?>
                    <tr class="<?php echo $row['complain_type'] == 'admin' ? '' : 'status_n'; ?>">
                        <td class="td_chk">
                            <input type="checkbox" class="complain_chk" name="chk[]" value="<?php echo $row['complain_idx']; ?>">
                        </td>
                        <td>
                            <?php
...
function fcomplainlist_submit(f) {
    ...
    if (document.pressed == "선택승인") {
        if (!confirm("선택한 회원을 승인하시겠습니까?")) { return false; }
    }
    return true;
}
</script>
