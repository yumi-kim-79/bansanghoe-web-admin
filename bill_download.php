...
// echo "SELECT ho.*, dong.dong_name FROM a_building_ho as ho LEFT JOIN a_building_dong as dong on ho.dong_id = dong.dong_id WHERE ho.ho_id = '{$ho_id}'";
...
$dong_name = $ho_info['dong_name'].'동';

$bill_item_info = "SELECT * FROM a_bill_item WHERE bill_id = '{$bill_info['bill_id']}' AND dong_name = '{$dong_name}' ORDER BY bi_idx ASC";
...
// $no_due_price_sum = $no_due_price + $no_due_price_penulty; 
$no_due_price_sum = (int)str_replace(",", "", $no_due_price) + (int)str_replace(",", "", $no_due_price_penulty); //미납금액 + 미납연체료 금액
    ...
    <div class="print_wrap">
        ...
        <div class="bill_due_info_wrap">
            <div class="bill_due_box">
                <div class="due_box red"><?php echo $due_in_price != '' ? $due_in_price : 0; ?></div> <!-- 납기내금액 -->
                <div class="due_box"><?php echo $bill_info['bill_due_date']; ?></div>
                <div class="due_box"><?php echo $due_out_price != '' ? $due_out_price : 0; ?></div> <!-- 납기후금액 -->
            ...
            <div class="bill_dear_box">
                ...
                <!-- 수신 주소 -->
                 ...
                 <?php
                    ...
                    echo $addr.'<br>'.$building_info['building_name'].' '.$dong_name.' '.$ho_info['ho_name'].'호';
                ?>
                 </div>
            </div>
        </div>
        ...
        <div class="bill_page2_wrap">
            <div class="bill_page2_left">
                ...
                <div class="bpage2_no_due_sum_wrap">
                    <div class="bpage2_no_due_sum"><?php echo $no_due_price_sum == 0 ? '' : number_format($no_due_price_sum); ?></div>
                </div>
            </div>
                ...
                <!-- 단지 공지사항 -->
                 ...
                 <div class="bill_right_last_wrap">
                    <div class="bill_last_left">
                        <div class="bill_last_left_box"><?php echo $due_price_total != '' ? $due_price_total : 0;?></div>
                        ...
                        <div class="bill_last_left_box red"><?php echo $due_in_price != '' ? $due_in_price : 0; ?></div>
                        <div class="bill_last_left_box"><?php echo $bill_info['bill_due_date']; ?></div>
                        <div class="bill_last_left_box"><?php echo $due_out_price_penulty != '' ? $due_out_price_penulty : 0; ?></div>
                        <div class="bill_last_left_box"><?php echo $due_out_price != '' ? $due_out_price : 0; ?></div>
                    ...
                    <div class="bill_last_right">
                        <div class="bill_last_right_box"><?php echo $due_in_price != '' ? $due_in_price : 0;?></div>
                        ...
                        <div class="bill_last_right_box"><?php echo $due_in_price != '' ? $due_in_price : 0;?></div>
                        <div class="bill_last_right_box"><?php echo $due_out_price_penulty != '' ? $due_out_price_penulty : 0;?></div>
                        <div class="bill_last_right_box"><?php echo $due_out_price != '' ? $due_out_price : 0; ?></div>
...
let admin_ip = "<?php echo ADMIN_IP; ?>";
$(function(){
   if(ip != admin_ip) bbsToImg();
});
...
function bbsToImg(){
    ...
    let building_name = '<?php echo $building_info['building_name']; ?>';
    let dong_name = '<?php echo $dong_name; ?>';
    let ho_name = '<?php echo $ho_info['ho_name']; ?>';
    let bill_date = '<?php echo $bill_info['bill_year'].'년 '.$bill_info['bill_month'].'월 관리비 고지서';?>';

    let fileName = building_name + '_' + dong_name + '_' + ho_name + '_' + bill_date;
    let filePath = '/data/file/billSample';

    let editor = document.querySelector('.print_wrap');
    ...
    html2canvas(editor, {
        ...
        $.ajax({
            url: '/bill_download_update.php', // 요청할 URL
            type: 'POST', // HTTP 메서드
            data: formData,
            cache: false,
            async: false,
            ...
            success: function(data) {
                ...
                if(data.result == false) { 
                    ...
                    //Linking 으로 열림
                    // sendMessage('bill', {"downloadUrl":"/bill_download_end.php?file_name=" + data.data});

                    sendMessage('building_file', {"url":filePath + '/' + data.data, "name":data.data});

                    setTimeout(() => {
                        location.href = "/bill.php";
                        // location.href = "/data/file/billSample/" + data.data;
                    }, 1500);
                }
